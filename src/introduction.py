...
class IntroduceNumberSystems(Slide):
    def construct(self):
        ...
        def show_arithmetic_binary_operation(val_a: int, operation: str, val_b: int, val_c: int, commutative = True):
            ...
            self.play(Write(tex[0:2]), Indicate(operand1_dot, color=BLUE, scale_factor=2), run_time=0.75)
            self.play(Write(tex[2:4]), Indicate(operand2_dot, color=BLUE if commutative else PURPLE, scale_factor=2), run_time=0.75)
            self.play(Write(tex[4]), Indicate(result_dot, color=GREEN, scale_factor=2), run_time=0.75)

            return tex

        temp_tex = show_arithmetic_binary_operation(5, '+', 8, 13)

        self.next_slide()
        self.play(FadeOut(temp_tex, shift=DOWN * 0.5), run_time=0.25)
        temp_tex = show_arithmetic_binary_operation(9, '-', 7, 2, False)

        self.next_slide()
        self.play(FadeOut(temp_tex, shift=DOWN * 0.5), run_time=0.25)
        temp_tex = show_arithmetic_binary_operation(7, r'\cdot', 2, 14)

        self.next_slide()
        self.play(FadeOut(temp_tex, shift=DOWN * 0.5), run_time=0.25)
        temp_tex = show_arithmetic_binary_operation(6, r'\div', 3, 2, False)
        ...
        pointer = Arrow(start=ORIGIN_NAT_NUMS + RIGHT * value_tracker.get_value() * SPACING + 2 * DOWN, end=ORIGIN_NAT_NUMS + RIGHT * value_tracker.get_value() * SPACING, buff=0.3)
        label = MathTex("x").add_updater(lambda m: m.next_to(pointer, DOWN))
        pointer_label = VGroup(pointer, label)

        pointer.add_updater(lambda m: m.move_to(ORIGIN_NAT_NUMS + RIGHT * value_tracker.get_value() * SPACING + DOWN))
